...
'\n'
'The teaching examples use small generated problems, because a 300-point scatter is the clearest way to *see* what boosting actually does. **Section 9 then runs the whole pipeline end to end on real data** -- the attached [Spotify Tracks: Audio Features (50K Songs)](https://www.kaggle.com/datasets/lorenzoscaturchio/spotify-tracks-audio-features-50k) dataset -- so you can watch the same techniques earn (or fail to earn) their keep on a real feature matrix.\n'
'\n'
...
cells.append(md(
'<a id="9"></a>\n'
'## 9. Practical Competition Pipeline (on real data)\n'
'\n'
'Everything above used generated toy problems on purpose -- a two-moons scatter is the clearest way to *see* what a booster is doing. This section drops the training wheels and runs the same pipeline on a **real, published dataset**: [Spotify Tracks: Audio Features (50K Songs)](https://www.kaggle.com/datasets/lorenzoscaturchio/spotify-tracks-audio-features-50k), attached to this notebook.\n'
'\n'
'### The task: pop vs. R&B\n'
'\n'
'Pop and R&B share a lot of production DNA -- overlapping tempo ranges, similar energy and danceability, the same studio conventions. Given **only** a track\'s audio features, can we tell them apart?\n'
'\n'
'That is a tabular classification problem with the shape of a real competition:\n'
'\n'
'| Property | Value |\n'
'|----------|-------|\n'
'| Task | Binary classification |\n'
'| Rows | ~10,000 tracks |\n'
'| Features | 15 (continuous, ordinal, and boolean) |\n'
'| Class balance | roughly 2:1, not 50/50 |\n'
'| Metric | AUC-ROC |\n'
'\n'
'It is deliberately **not** a separable problem. Some genre pairs in this dataset -- jazz vs. hip-hop, say -- are close to trivially separable, and a trivial problem teaches you nothing about ensembling: every model saturates and the blend has no room to help. Pop vs. R&B leaves real headroom, which is exactly where blend weights and a meta-learner have something to prove.\n'
'\n'
'Two things change once the data is real, and both are the point:\n'
'\n'
'1. **Feature scales stop being uniform.** `duration_ms` lives in the hundreds of thousands, `valence` lives in $[0, 1]$, `release_year` sits around 2015. Trees do not care. Logistic regression and the MLP care enormously, so both get a `StandardScaler` fitted **inside each fold** -- never on the full training set, which is how scaling leaks.\n'
'2. **The gains get small.** On `make_classification` an ensemble can look heroic. On real data a well-tuned blend usually buys you the third or fourth decimal place of AUC -- and on a Kaggle leaderboard, that is frequently the whole margin.\n'
'\n'
'### Pipeline Overview\n'
'\n'
'```\n'
...
cells.append(code(
'# ============================================================\n'
'# FULL COMPETITION PIPELINE -- REAL DATA\n'
'# ============================================================\n'
'\n'
'# --- Step 1: Load the attached dataset ---\n'
'# Dataset: lorenzoscaturchio/spotify-tracks-audio-features-50k\n'
'# On Kaggle it mounts under /kaggle/input/; the other candidates let the\n'
'# same cell run from a local checkout of the repo this notebook lives in.\n'
'CANDIDATE_PATHS = [\n'
'    "/kaggle/input/spotify-tracks-audio-features-50k/spotify_tracks.csv",\n'
'    "../input/spotify-tracks-audio-features-50k/spotify_tracks.csv",\n'
'    "datasets/spotify-tracks/spotify_tracks.csv",\n'
'    "../../../datasets/spotify-tracks/spotify_tracks.csv",\n'
']\n'
'csv_path = next((p for p in CANDIDATE_PATHS if os.path.exists(p)), None)\n'
'if csv_path is None:\n'
'    raise FileNotFoundError(\n'
'        "spotify_tracks.csv not found. Attach the dataset to this notebook: "\n'
'        "Add Data -> search \'Spotify Tracks: Audio Features (50K Songs)\'."\n'
'    )\n'
'\n'
'tracks = pd.read_csv(csv_path)\n'
'print(f"Loaded {len(tracks):,} tracks x {tracks.shape[1]} columns")\n'
'print(f"Source: {csv_path}")\n'
'\n'
'# --- Step 1b: Frame the binary task (pop vs. R&B) ---\n'
'GENRE_POS, GENRE_NEG = "pop", "r&b"\n'
'FEATURES = [\n'
'    # audio features\n'
...
'    # track metadata\n'
'    "release_year", "duration_ms", "explicit",\n'
']\n'
'\n'
'pair = tracks[tracks["genre"].isin([GENRE_POS, GENRE_NEG])].reset_index(drop=True)\n'
'feat = pair[FEATURES].copy()\n'
'feat["explicit"] = feat["explicit"].astype(int)   # bool -> 0/1\n'
'\n'
'X_full = feat.to_numpy(dtype=float)\n'
'y_full = (pair["genre"] == GENRE_POS).astype(int).to_numpy()\n'
'\n'
'print(f"\\nTask: {GENRE_POS} (1) vs {GENRE_NEG} (0)")\n'
'print(f"Rows: {len(y_full):,} | Features: {X_full.shape[1]} | "\n'
'      f"Positive rate: {y_full.mean():.3f}")\n'
'print("\\nFeature scales (why linear/NN models need scaling and trees do not):")\n'
'print(feat.describe().T[["mean", "std", "min", "max"]].round(2).to_string())\n'
'\n'
'# --- Step 1c: Hold out a test set (in a competition, test has no labels) ---\n'
'# Split on indices too, so the submission can carry the real track_ids.\n'
'idx_train, idx_test = train_test_split(\n'
'    np.arange(len(y_full)), test_size=0.2, stratify=y_full, random_state=SEED\n'
')\n'
'X_train_full, X_test_full = X_full[idx_train], X_full[idx_test]\n'
'y_train_full, y_test_full = y_full[idx_train], y_full[idx_test]\n'
'print(f"\\nTrain: {X_train_full.shape}, Test: {X_test_full.shape}")\n'
'\n'
...
'        model = clone(model_template)\n'
'\n'
'        # Scale inside the fold for the models that need it. Fitting the\n'
'        # scaler on the whole training set would leak the validation\n'
'        # fold\'s distribution into the features it is scored on.\n'
'        if name in ("LR", "MLP"):\n'
'            sc = StandardScaler()\n'
...
'\n'
'# Create the submission file, keyed on the real track_ids of the held-out rows\n'
'submission = pd.DataFrame({\n'
'    "track_id": pair.loc[idx_test, "track_id"].to_numpy(),\n'
'    "target": final_blend,\n'
'})\n'
'submission.to_csv("submission.csv", index=False)\n'
'print(f"\\nSubmission shape: {submission.shape} -> submission.csv")\n'
...
'>\n'
'> Read the three numbers printed above against each other, not in isolation. On real data the optimized blend and the stacked meta-learner tend to land within a whisker of one another and a few thousandths of AUC above the simple average, and the fitted weight vector usually comes out lopsided rather than uniform -- the models are not equally good and the optimizer knows it. A margin that small is unremarkable in a tutorial and decisive on a leaderboard. It is also why the discipline matters: those weights were fitted on out-of-fold predictions, so the gain has a fair chance of surviving contact with the private test set.\n'
'\n'
'---'
))
