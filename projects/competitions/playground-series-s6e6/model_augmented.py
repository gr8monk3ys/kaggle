...
def main() -> int:
    ...
    print(
        f"synthetic train={train.shape} test={test.shape} | original={orig.shape}",
        flush=True,
    )
    ...
    def build(df):
        return np.hstack(
            [df[numeric_feats].to_numpy(float), enc.transform(df[CAT].astype(str))]
        )
    ...
    for fold, (tri, vai) in enumerate(skf.split(X, y), 1):
        ...
        for name, make in builders.items():
            ...
            test_pred[name] += (
                model.predict_proba(X_test).reshape(len(X_test), n_classes) / 5
            )
        blend = np.mean([oof[k][vai] for k in builders], axis=0)
        print(
            f"  fold {fold}: blend macroF1={f1_score(y[vai], blend.argmax(1), average='macro'):.5f} "
            f"({time.time() - tf:.0f}s)",
            flush=True,
        )
    ...
    print(
        ...
        + ", ".join(
            f"{k}={f1_score(y, oof[k].argmax(1), average='macro'):.5f}"
            for k in builders
        ),
        flush=True,
    )
    print(
        f"blend-avg OOF macroF1 = {avg_f1:.5f}  (F1-tuned {f1_avg_t:.5f})", flush=True
    )
    print(
        f"stack    OOF macroF1 = {stack_f1:.5f}  (F1-tuned {f1_stk_t:.5f})", flush=True
    )

    best_name = max(candidates, key=lambda k: candidates[k][0])
    best_f1, best_test = candidates[best_name]
    print(
        f"\n=== BEST = {best_name}  OOF macroF1 = {best_f1:.5f}  (baseline 0.957) ===",
        flush=True,
    )
    ...
    print(
        f"Wrote {OUT} ({len(sub):,} rows)  class mix: {sub[TARGET].value_counts().to_dict()}",
        flush=True,
    )
