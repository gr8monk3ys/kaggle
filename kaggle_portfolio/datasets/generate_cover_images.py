...
def gen_ai_research(data_dir):
    ...
    if "year" in df.columns:
        ...
        style_ax(
            ax, "AI/ML Research Papers Published Per Year", "Year", "Number of Papers"
        )
...
def gen_credit_card_fraud(data_dir):
    ...
    fraud_col = next(
        (c for c in df.columns if "fraud" in c.lower() or "class" in c.lower()), None
    )
    if fraud_col:
        ...
        ax1.pie(
            ...
            startangle=90,
            explode=[0, 0.1] if len(counts) == 2 else None,
        )
...
def gen_ecommerce(data_dir):
    ...
    spend_col = next(
        (
            c
            for c in df.columns
            if "spend" in c.lower() or "total" in c.lower() or "revenue" in c.lower()
        ),
        None,
    )
    seg_col = next(
    ...
    if spend_col and seg_col:
        ...
        if top_cols:
            ...
            ax.barh(
                counts.index[::-1],
                counts.values[::-1],
                color=ACCENT_COLORS[: len(counts)],
            )
...
def gen_github(data_dir):
    ...
    if stars_col and forks_col and lang_col:
        ...
        style_ax(
            ax, "GitHub Repos: Stars vs Forks by Language", "Stars (log)", "Forks (log)"
        )
    elif stars_col:
        ax.hist(
            df[stars_col].clip(upper=df[stars_col].quantile(0.95)),
            bins=50,
            color=ACCENT_COLORS[0],
            alpha=0.7,
            edgecolor="#333",
...
def gen_job_postings(data_dir):
    ...
    salary_col = next(
        (
            ...
            and ("max" in c.lower() or "avg" in c.lower() or "mid" in c.lower())
        ),
        None,
    )
    if not salary_col:
    ...
    title_col = next(
        (c for c in df.columns if "title" in c.lower() or "role" in c.lower()), None
    )
    if salary_col and title_col:
        ...
        ax.xaxis.set_major_formatter(
            ticker.FuncFormatter(lambda x, p: f"${x / 1000:.0f}K")
        )
        style_ax(ax, "Median Salary by Job Title (Top 12)", "Salary", "")
    elif salary_col:
        ax.hist(
            df[salary_col].dropna(),
            bins=40,
            color=ACCENT_COLORS[0],
            alpha=0.7,
            edgecolor="#333",
...
def gen_mental_health(data_dir):
    ...
    fig, ax = plt.subplots(figsize=FIG_SIZE)
    treat_col = next(
        (c for c in df.columns if "treatment" in c.lower() or "sought" in c.lower()),
        None,
    )
    age_col = next((c for c in df.columns if "age" in c.lower()), None)
    if treat_col and age_col:
        df["_treat_num"] = (
            df[treat_col].map({"Yes": 1, "No": 0, 1: 1, 0: 0}).fillna(0).astype(float)
        )
        ...
        ax.bar(
            [str(x) for x in rates.index],
            rates.values,
            color=ACCENT_COLORS[: len(rates)],
            alpha=0.8,
        )
        style_ax(
            ax,
            "Mental Health Treatment Rates by Age Group",
            "Age Group",
            "Sought Treatment (%)",
        )
    else:
...
def gen_ml_interview(data_dir):
    ...
    fig, ax = plt.subplots(figsize=FIG_SIZE)
    cat_col = next(
        (c for c in df.columns if "category" in c.lower() or "topic" in c.lower()), None
    )
    diff_col = next(
        (c for c in df.columns if "difficulty" in c.lower() or "level" in c.lower()),
        None,
    )
    if cat_col and diff_col:
        ...
        ax.legend(
            facecolor=BG_COLOR, edgecolor="#333", labelcolor=TEXT_COLOR, fontsize=9
        )
        style_ax(ax, "ML Interview Questions: Category x Difficulty", "Count", "")
    elif cat_col:
        counts = df[cat_col].value_counts().head(10)
        ax.barh(
            counts.index[::-1], counts.values[::-1], color=ACCENT_COLORS[: len(counts)]
        )
...
def gen_programming_benchmarks(data_dir):
    ...
    time_col = next(
        (
            c
            for c in df.columns
            if "time" in c.lower() or "speed" in c.lower() or "perf" in c.lower()
        ),
        None,
    )
    if lang_col and time_col:
        ...
        colors = [
            ACCENT_COLORS[i % len(ACCENT_COLORS)] for i in range(len(median_perf))
        ]
...
def gen_spotify(data_dir):
    ...
    if energy_col and dance_col and pop_col:
        ...
        if not filtered.empty:
            ...
            style_ax(
                ax, "Spotify Tracks: Energy vs Danceability", "Energy", "Danceability"
            )
    elif energy_col:
        ax.hist(
            df[energy_col].dropna(),
            bins=50,
...
def gen_student_performance(data_dir):
    ...
    gpa_col = next(
        (
            c
            for c in df.columns
            if "gpa" in c.lower() or "grade" in c.lower() or "score" in c.lower()
        ),
        None,
    )
    study_col = next(
        (c for c in df.columns if "study" in c.lower() or "hours" in c.lower()), None
    )
    if gpa_col and study_col:
        ...
        if not filtered.empty:
            ...
            ax.plot(
                x_line, p(x_line), color=ACCENT_COLORS[3], linewidth=2.5, linestyle="--"
            )
            style_ax(
                ax,
                "Study Hours vs GPA (with Trend Line)",
...
def gen_ai_data_jobs(data_dir):
    ...
    fig, ax = plt.subplots(figsize=FIG_SIZE)
    med = df.groupby("role_family")["salary_mid_usd"].median().sort_values().tail(10)
