...
def score_dataset(
    dir_path: Path, meta: dict, votes: int, downloads: int
) -> dict[str, Any]:
    ...
    if len(keywords) < MIN_KEYWORDS:
        actions.append(
            f"add {MIN_KEYWORDS - len(keywords)} keyword(s) for search (have {len(keywords)})"
        )
    if not has_notebook:
...
def fetch_live_stats(
    client: KaggleClient, owner: str | None
) -> dict[str, dict[str, int]]:
    """Best-effort ``slug -> {'votes','downloads'}``; ``{}`` when Kaggle is unreachable."""
    try:
        datasets = client.datasets_owned_by(owner) if owner else client.my_datasets()
    except KaggleError as exc:
        print(f"{YELLOW}live stats unavailable{RESET}: {exc}", file=sys.stderr)
        return {}
    return {
        d.slug: {"votes": d.vote_count, "downloads": d.download_count}
        for d in datasets
        if d.slug
    }
...
def build_plan(
    datasets: list[tuple[Path, dict]], stats: dict[str, dict[str, int]]
) -> list[dict]:
    rows = []
    ...
    rows.sort(
        key=lambda r: (r["votes_to_next"] == 0, r["votes_to_next"], -r["downloads"])
    )
    return rows
...
def main(argv: list[str] | None = None, deps: Deps | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan dataset votes toward the next medal."
    )
    parser.add_argument(
        "--owner", default=None, help="Kaggle owner slug (default: your own datasets)."
    )
    parser.add_argument("--json", action="store_true", help="Machine-readable output.")
    args = parser.parse_args(argv)
    deps = deps or Deps.resolve()

    plan = build_plan(discover_datasets(), fetch_live_stats(deps.client, args.owner))
    if args.json:
    ...
    print(
        f"Dataset vote plan ({len(plan)} datasets){'' if has_live else '  [no live stats - run with creds]'}\n"
    )
    for r in plan:
        marker = (
            f"{GREEN}[gold]{RESET}"
            if r["votes_to_next"] == 0
            else f"-> {r['next_medal']}"
        )
