...
MOUNT_GUESS = '/kaggle/input/spotify-tracks-audio-features-50k'
MARKER = 'spotify_tracks.csv'
...
# No synthetic fallback. This notebook once manufactured 50,000 fake tracks when the
# dataset was missing and rendered every figure and metric from them, under a title
# claiming real Spotify data -- the warning was two stdout lines a reader scrolling to
# a chart would never see. `dataset_sources` attaches the dataset on Kaggle, so the
# fallback guarded a case that should not arise, at the cost of publishing invented
# numbers as findings. Failing with the resolver's instructions is the honest stop.
DATA_DIR = _find_data_dir(MARKER)
csv_path = Path(DATA_DIR) / MARKER
...
cells.append(md("## 7. Mood Clustering (UMAP + K-Means) <a id='clustering'></a>\n\n"
                "*The plot below is labelled with whichever reducer actually ran. Kaggle has\n"
                "`umap-learn`, so it shows UMAP there; without it the cell falls back to PCA\n"
                "and says so in the title, rather than presenting PCA output under a UMAP heading.*"))
...
**Audio Features:**
- **Energy** and **loudness** move together moderately (r = 0.58) — louder does mean
  more energetic, but the relationship is looser than it looks; the heatmap above is
  the measured value, not a remembered one
- **Acousticness** and **energy** are strongly negatively correlated (r = -0.68) — acoustic tracks are quieter
