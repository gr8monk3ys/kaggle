...
INDUSTRIES = {
    "software": {
        "weight": 0.22,
        "salary_mult": 1.10,
        "company_types": ["product", "platform", "developer-tools"],
    },
    "finance": {
        "weight": 0.14,
        "salary_mult": 1.16,
        "company_types": ["fintech", "bank", "asset-manager"],
    },
    "healthcare": {
        "weight": 0.10,
        "salary_mult": 1.05,
        "company_types": ["healthtech", "provider", "biotech"],
    },
    "cloud": {
        "weight": 0.10,
        "salary_mult": 1.11,
        "company_types": ["cloud-platform", "infra", "security"],
    },
    "retail": {
        "weight": 0.10,
        "salary_mult": 0.95,
        "company_types": ["marketplace", "commerce", "consumer"],
    },
    "consulting": {
        "weight": 0.10,
        "salary_mult": 1.02,
        "company_types": ["consulting", "services", "advisory"],
    },
    "media": {
        "weight": 0.08,
        "salary_mult": 0.93,
        "company_types": ["media", "streaming", "adtech"],
    },
    "public-sector": {
        "weight": 0.06,
        "salary_mult": 0.92,
        "company_types": ["public-sector", "research-consortium"],
    },
    "education": {
        "weight": 0.05,
        "salary_mult": 0.90,
        "company_types": ["edtech", "learning-platform"],
    },
    "manufacturing": {
        "weight": 0.05,
        "salary_mult": 0.97,
        "company_types": ["industrial", "automation"],
    },
}
...
ROLE_WEIGHTS = np.array(
    [ROLE_CONFIGS[name]["weight"] for name in ROLE_NAMES], dtype=float
)
...
def generate_companies() -> pd.DataFrame:
    ...
    industry_weights = np.array(
        [INDUSTRIES[name]["weight"] for name in industries], dtype=float
    )
    ...
    for idx in range(N_COMPANIES):
        ...
        remote_first = bool(
            rng.random() < (0.45 if size in {"startup", "small"} else 0.24)
        )
        ai_maturity = float(
            np.clip(
                rng.normal(
                    69 if industry in {"software", "cloud", "finance"} else 58, 12
                ),
                20,
                98,
            )
        ...
        benefits_score = float(
            np.clip(
                rng.normal(72 if size in {"large", "enterprise"} else 63, 10), 35, 96
            )
        )
        glassdoor_rating = float(
            np.clip(
                rng.normal(3.9 if size in {"large", "enterprise"} else 3.7, 0.35),
                2.7,
                4.9,
            )
        )
        hiring_velocity = int(
            np.clip(rng.normal(68 if remote_first else 56, 14), 20, 99)
        )
        rows.append(
            {
...
def pick_skills(
    ...
    if family in {"genai-apps", "research"} or focus_area in {
        "rag",
        "agents",
        "fine-tuning",
        "evaluation",
...
def generate_jobs(companies: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for idx in range(N_JOBS):
        ...
        remote_type = str(
            rng.choice(
                ["onsite", "hybrid", "remote"], p=remote_probs / remote_probs.sum()
            )
        )
        ...
        llm_stack = (
            pick_llm_stack()
            if role_cfg["family"] in {"genai-apps", "research"}
            or focus_area in {"rag", "agents", "fine-tuning", "evaluation"}
            else []
        )
        skills = pick_skills(
            role, role_cfg["family"], focus_area, cloud_stack, llm_stack
        )
        ...
        location_match = next(
            item for item in LOCATIONS if item["city"] == company["hq_city"]
        )
        ...
        equity_offered = int(
            ...
            < (
                0.72
                if str(company["company_size"]) in {"startup", "small"}
        ...
        bonus_target = (
            role_cfg["bonus_base"]
            + (0.03 if company["industry"] == "finance" else 0.0)
            + (0.02 if seniority in {"staff", "principal"} else 0.0)
        )
        ...
        visa_sponsorship = int(
            ...
            < (
                0.34
                if str(company["company_size"]) in {"large", "enterprise"}
                and company["country"]
        ...
        hiring_urgency = (
            "high" if days_open <= 18 else ("medium" if days_open <= 40 else "low")
        )
        ...
        for skill_idx, skill in enumerate(skills):
            ...
            importance = (
                "core"
                if skill_idx < 3
                else ("strong" if skill_idx < 6 else "nice_to_have")
            )
            proficiency = (
                "advanced"
                if skill_idx < 3
                else ("intermediate" if skill_idx < 6 else "basic")
            )
            skill_rows.append(
                {
                    "job_id": job_id,
...
def build_salary_benchmarks(jobs: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        jobs.groupby(
            ["job_title", "country", "seniority", "remote_type"], observed=False
        )
        .agg(
            sample_jobs=("job_id", "count"),
            salary_p10_usd=(
                "salary_mid_usd",
                lambda s: int(round(np.quantile(s, 0.10))),
            ),
            salary_median_usd=("salary_mid_usd", lambda s: int(round(np.median(s)))),
            salary_p90_usd=(
                "salary_mid_usd",
                lambda s: int(round(np.quantile(s, 0.90))),
            ),
            bonus_target_pct_median=(
                "bonus_target_pct",
                lambda s: round(float(np.median(s)), 3),
            ),
    ...
    return (
        grouped[grouped["sample_jobs"] >= 15]
        .sort_values(["job_title", "country", "seniority", "remote_type"])
        .reset_index(drop=True)
    )


def build_skill_demand_monthly(
    jobs: pd.DataFrame, job_skills: pd.DataFrame
) -> pd.DataFrame:
    jobs_with_month = jobs[
        ["job_id", "posted_date", "salary_mid_usd", "remote_type"]
    ].copy()
    jobs_with_month["year_month"] = (
        pd.to_datetime(jobs_with_month["posted_date"]).dt.to_period("M").astype(str)
    )
    ...
    monthly_jobs = (
        jobs_with_month.groupby("year_month", observed=False)["job_id"]
        .nunique()
        .rename("month_jobs")
    )
    grouped = (
        ...
        .agg(
            ...
            median_salary_mid_usd=(
                "salary_mid_usd",
                lambda s: int(round(np.median(s))),
            ),
            remote_share=(
                "remote_type",
                lambda s: round(float((s == "remote").mean()), 3),
            ),
        )
        .reset_index()
    )
    grouped = grouped.merge(monthly_jobs.reset_index(), on="year_month", how="left")
    grouped["share_of_postings"] = (grouped["job_count"] / grouped["month_jobs"]).round(
        4
    )
    grouped = grouped.drop(columns=["month_jobs"])
    return grouped.sort_values(
        ["year_month", "job_count", "skill"], ascending=[True, False, True]
    ).reset_index(drop=True)
...
def main() -> None:
    ...
    print(
        f"  companies.csv            {len(companies):>8,} rows x {companies.shape[1]} cols"
    )
    print(f"  jobs.csv                 {len(jobs):>8,} rows x {jobs.shape[1]} cols")
    print(
        f"  job_skills.csv           {len(job_skills):>8,} rows x {job_skills.shape[1]} cols"
    )
    print(
        f"  salary_benchmarks.csv    {len(salary_benchmarks):>8,} rows x {salary_benchmarks.shape[1]} cols"
    )
    print(
        f"  skill_demand_monthly.csv {len(skill_demand):>8,} rows x {skill_demand.shape[1]} cols"
    )
