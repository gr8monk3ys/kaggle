...
BENCHMARKS = [
    ...
    "multiple established benchmarks",
    "diverse evaluation suites",
    "large-scale datasets",
    "both synthetic and real-world data",
    "competitive benchmarks",
    "widely-used evaluation protocols",
]
...
def generate_abstract():
    ...
    abstract = template.format(
        ...
        component=random.choice(
            ["attention", "gating", "fusion", "routing", "pooling"]
        ),
...
def main():
    ...
    while len(papers) < NUM_PAPERS:
        year = random.choices(
            list(range(2022, 2026)), weights=[0.25, 0.30, 0.28, 0.17], k=1
        )[0]
        papers.append(generate_paper(paper_id, year))
        paper_id += 1
