...
Usage
-----
    python3 discussion_scheduler.py --dry-run     # print next 3 queued drafts
    python3 discussion_scheduler.py               # post next ready draft via Playwright
    python3 discussion_scheduler.py --init        # (re-)generate queue from drafts file
    python3 discussion_scheduler.py --init --schedule-weeks 4
                                                 # keep a rolling 4-week scheduled window
    python3 discussion_scheduler.py --ops-report  # show stage + backlog priority summary
    python3 discussion_scheduler.py --health-check
                                                 # fail on stale schedule SLAs
    python3 discussion_scheduler.py --set-id draft_007 --priority high --deadline 2026-03-05
...
from kaggle_utils import parse_iso_date
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.init or not QUEUE_FILE.exists():
    ...
    if args.set_id:
        if not any([args.status, args.priority, args.deadline, args.clear_deadline]):
            parser.error("--set-id requires at least one of --status/--priority/--deadline/--clear-deadline.")
