...
def run_kaggle_csv(args: list[str]) -> tuple[list[dict[str, str]], list[str]]:
    output = run_checked_command(["kaggle", *args, "--csv"])
...
def fetch_live_kaggle_metrics() -> dict[str, Any]:
    ...
    kernels_rows, kernels_columns = run_kaggle_csv(["kernels", "list", "--mine", "--page-size", "100"])
    datasets_rows, datasets_columns = run_kaggle_csv(["datasets", "list", "-m", "--page-size", "100"])
    featured_rows, featured_columns = run_kaggle_csv(
        ["competitions", "list", "--category", "featured", "--page-size", "100"]
    )
    research_rows, research_columns = run_kaggle_csv(
        ["competitions", "list", "--category", "research", "--page-size", "100"]
    )
    competition_columns = list(dict.fromkeys([*featured_columns, *research_columns]))
    ...
    entered_total, entered_key = parse_entered_total(
        featured_rows + research_rows,
        competition_columns,
        "kaggle competitions list output",
        strict=False,
    )
...
kaggle datasets list -m --page-size 100 --csv > "${OUT_DIR}/datasets.csv"

featured_tmp="${OUT_DIR}/competitions-featured.tmp.csv"
research_tmp="${OUT_DIR}/competitions-research.tmp.csv"

kaggle competitions list --category featured --page-size 100 --csv > "${featured_tmp}"
kaggle competitions list --category research --page-size 100 --csv > "${research_tmp}"

if [[ -s "${featured_tmp}" ]]; then
  head -n 1 "${featured_tmp}" > "${OUT_DIR}/competitions.csv"
  tail -n +2 "${featured_tmp}" >> "${OUT_DIR}/competitions.csv"
  if [[ -s "${research_tmp}" ]]; then
    tail -n +2 "${research_tmp}" >> "${OUT_DIR}/competitions.csv"
  fi
elif [[ -s "${research_tmp}" ]]; then
  cp "${research_tmp}" "${OUT_DIR}/competitions.csv"
else
  printf 'competition,userHasEntered\\n' > "${OUT_DIR}/competitions.csv"
fi

rm -f "${featured_tmp}" "${research_tmp}"
...
This folder was generated by `python3 medal_ops.py sync-template`.
...
- `competitions.csv`: optional for sync; expected to include `userHasEntered`.
...
def has_kaggle_credentials() -> tuple[bool, list[Path]]:
...
def run_preflight_checks(
    ...
    if kaggle_cli_available:
        infos.append("kaggle CLI found in PATH.")
    else:
        if require_kaggle:
            errors.append("kaggle CLI not found in PATH.")
        elif offline_mode:
...
def parse_args() -> argparse.Namespace:
    ...
    subparsers.add_parser("scorecard", help="Generate scorecard report and save snapshot.")
    subparsers.add_parser("weekly-plan", help="Generate weekly execution plan.")
    subparsers.add_parser("pace", help="Generate progress velocity and ETA analysis.")
