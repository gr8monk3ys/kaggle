...
-----
    ...
    python3 notebook_pipeline.py --stale-only

    # Build + push notebooks scoring >= 60 (default threshold)
    python3 notebook_pipeline.py --push

    # Validate metadata only, no build
    python3 notebook_pipeline.py --validate-only
...
from kaggle_utils import kaggle_command
...
def score_notebook_dir(nb_dir: Path) -> tuple[float, str]:
    """Run notebook_quality.py on a directory and return (score, summary)."""
